...
class MyBot:
    ...
    async def send_message(self, chat_id: int, text: str): #функция для логов по совместительству обработчик ошибок при отправке сообщения, оборачивает все отправления сообщений
        try:
    ...
    async def start_handler(self, message: Message): #/start приветственное сообщение
        await message.answer("Привет! Я бот для запуска скриптов по расписанию.")

    async def help_handler(self, message: Message):#/help
        help_text = """
    ...
    async def run_script_handler(self, message: Message):# /run_script принудительное включение скрипта не смотря на расписание
    ...
    async def list_scripts_handler(self, message: Message): #list_scripts просто список добавленых скриптов, отсюда удобно брать названия
    ...
    async def add_script_handler(self, message: Message): #/add_script добавления скрипта в расписание, нужно укзать название и крон через пробел
        args = message.text.split()[1:]
        if len(args) < 2:
    ...
    async def remove_script_handler(self, message: Message): #remove_script удаляет скрипт из расписания по названию
    ...
    async def start(self):
        ...
        self.scheduler.add_script("hello_world", "0 12 * * *") # добавил скрипт в автозапуск, чтобы не добавлять его через бота
...
if __name__ == "__main__":
    bot = MyBot() #объявление экземпляра класса
    
    import asyncio
    try: #обработчик для безопасного запуска и остановки бота
